    ...
    <div class="nav-bar">
        <header>
            ...
            <nav class="top-right-menu">
                <ul>
                    <li class="active-nav"><a href="{{ url('/admin_dashboard') }}">Home</a></li>
            ...
            <section class="statistics-dashboard">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card enrolled">
                        <h3>Unique Enrolled Students</h3>
                        <p>{{ $stats['enrolled'] }}</p>
                    </div>
                    
                    <div class="stat-card users-summary">
                        <h3>All Active Users</h3>
                        <p>{{ $stats['users'] }}</p>
                    </div>
                    
                    <div class="center-card">
                        <h3>Total Enrollment Records</h3>
                        <p>{{ $stats['total_enrollment_records'] }}</p> 
                    </div>

                    <div class="stat-card user-distribution-card">
                        <h3>User Role Distribution</h3>
                        <div class="progress-bar-container">
                            <div class="progress-bar-faculty" style="width: {{ $stats['faculty_percent'] }}%;">
                                @if($stats['faculty_percent'] > 10) {{ $stats['faculty_percent'] }}% @endif
                            </div>
                            <div class="progress-bar-student" style="width: {{ max($stats['students_percent'], 1) }}%;">
                                @if($stats['students_percent'] > 10) {{ $stats['students_percent'] }}% @endif
                            </div>
                        </div>
                        ...
                        <div class="legend">
                            <div class="legend-item">
                                ...
                                Faculty: {{ $stats['faculty'] }} ({{ $stats['faculty_percent'] }}%)
                            </div>
                            <div class="legend-item">
                                <span class="legend-color" style="background-color: #cb7d10ff;"></span>
                                Students: {{ $stats['students'] }} ({{ $stats['students_percent'] }}%)
                            </div>
                        </div>
                    </div>
                </div>
