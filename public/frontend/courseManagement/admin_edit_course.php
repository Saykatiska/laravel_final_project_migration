...
if ($course_id <= 0) {
    header("Location: admin_course_management.php?message=" . urlencode("Invalid course ID.") . "&status=error");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($course_code) && !empty($course_name) && !empty($faculty_id)) {
        ...
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM courses WHERE course_code = ? AND course_id != ?");
        ...
        if ($count > 0) {
            // Duplicate found
            $message = "Error: The course code '{$course_code}' is already used by another course. Please use a unique code.";
            $status = "error";
        } else {
            // Update the record
            $stmt = $conn->prepare("UPDATE courses SET course_code = ?, course_name = ?, faculty_id = ? WHERE course_id = ?");
            $stmt->bind_param("ssii", $course_code, $course_name, $faculty_id, $course_id);

            if ($stmt->execute()) {
                $message = "Success: Course " . htmlspecialchars($course_name) . " has been updated successfully.";

                $status = "success";
            } else {
                $message = "Error: Could not update course. " . $stmt->error;
                $status = "error";
            }
...
if (!$course) {
    header("Location: admin_course_management.php?message=" . urlencode("Course not found.") . "&status=error");
    exit();
}
...
// Close the connection if it wasn't closed in the POST block
if (isset($conn)) {
    close_db_connection($conn);
}
?>
...
<body>
    ...
    <main class="main">
        <section class="form-container-box">
            <h1>Edit Course: <?php echo htmlspecialchars($course['course_code']); ?></h1>
            ...
            <form method="POST" action="">
                ...
                <input type="text" id="course_code" name="course_code" value="<?php echo htmlspecialchars($course['course_code']); ?>" required><br>

                <label for="course_name">Course Name:</label>
                <input type="text" id="course_name" name="course_name" value="<?php echo htmlspecialchars($course['course_name']); ?>" required><br>
                ...
                <select id="faculty_id" name="faculty_id" required>
                    ...
                    <?php foreach ($faculty_members as $faculty): ?>
                        <option value="<?php echo $faculty['user_id']; ?>" 
                            <?php if ($course['faculty_id'] == $faculty['user_id']) echo 'selected'; ?>>
