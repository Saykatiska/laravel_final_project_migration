...
function validate_and_sanitize_input($post_data) {
    // Sanitize Input
    $data = [
    ...
    // Validate Input
    if (empty($data['username']) || empty($post_data['password']) || empty($data['role']) || empty($data['first_name']) || empty($data['last_name']) || empty($data['email'])) {
        return "Error: All fields are required.";
    } 
    
    if (!in_array($data['role'], ['Faculty', 'Student'])) {
...
// Checks the database for existing users with the given username or email
...
function insert_new_user($data) {
    try {
        ...
        return "Success: New user <strong>" . htmlspecialchars($data['username']) . "</strong> created successfully!";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $validation_result = validate_and_sanitize_input($_POST);
    
    if (is_string($validation_result)) {
        // Handle validation/sanitization error message
        $message = $validation_result;
    } else {
        // Validation/Sanitization was successful, result is the data array
        $new_user_data = $validation_result;
        
        // Check for existence
        if (check_user_exists($new_user_data['username'], $new_user_data['email'])) {
            $message = "Error: Username or Email already exists. Please use a different one.";
        } else {
            // Insert the user
            $message = insert_new_user($new_user_data);
        }
    }
}
...
<body>
    ...
    <main class="main">
        <section class="form-container-box">
            ...
            <br>
            <br><a href="admin_user_management.php">
